...
SearchTwitterAssistant.prototype.search = function(e, type, since_id) {
	var thisA = this;
	var search_string;

	type = type ? type.toLowerCase() : "";

	if (type == 'refresh') { // empty unless this is a refresh
		this.scrollToTop();
		this.markAllAsRead();
	...
	function getSearchTimeline(statusobj) {
		...
		if ( statusobj.isInternetConnectionAvailable === true) {
			...
			thisA.twit.search(
				...
				since_id ? ('-' + since_id) : null, // since_id -> max_id
				10, // results_per_page
				null, // page
				null, // lang
				null, // , geocode
				function(data, searchinfo) { // onSuccess
					...
					thisA.max_id = searchinfo.max_id;
...
SearchTwitterAssistant.prototype.loadMore = function(event) {
	this.search(this.searchBoxModel.value, 'loadmore', this.max_id);
};
...
SearchTwitterAssistant.prototype.activateSpinner = function(type) {
	if (type == 'loadmore') {
		var buttonWidget = this.controller.get('more-search-button');
		buttonWidget.mojo.activate();
	} else {
		var buttonWidget = this.controller.get('submit-search-button');
		buttonWidget.mojo.activate();
	}
};

SearchTwitterAssistant.prototype.deactivateSpinner = function(type) {
	if (type == 'loadmore') {
		var buttonWidget = this.controller.get('more-search-button');
